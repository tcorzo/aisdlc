...
只要任务会读写以下路径（或其子路径），必须先执行 `spec-context` 并回显 `FEATURE_DIR=...`。失败即停止。
...
- `spec-context` 失败或无法获得关键上下文（`FEATURE_DIR/REPO_ROOT/CURRENT_BRANCH` 等）
...
2. **门禁**：若下一步会读写 `{FEATURE_DIR}/...` 或写 demo → 先 `spec-context`，回显 `FEATURE_DIR=...`，失败即停止（硬中断）。
