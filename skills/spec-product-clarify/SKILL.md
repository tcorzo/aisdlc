...
将 `{FEATURE_DIR}/requirements/raw.md` 的原始输入，收敛为可评审的决策文档 `{FEATURE_DIR}/requirements/solution.md`；澄清过程必须可追溯（回写到 `raw.md/## 澄清记录`）。
...
- 写：`{FEATURE_DIR}/requirements/raw.md` 仅追加/更新 `## 澄清记录`；`{FEATURE_DIR}/requirements/solution.md` 为唯一决策入口
...
4. 同步更新 `solution.md`：推荐/备选差异、证据入口、残余未知 → 验证清单
...
> 回写必须发生在“拿到用户回答”之后；不要在 `raw.md` 里写“待用户填写/占位符”。
...
若明确跳过 R2（单独 PRD），在 `solution.md` 追加 **Mini-PRD**：
