...
- **不要用在**
  ...
  - PRD 明确判定“可跳过 R3”（无交互变化/交互简单明确）→ **不要生成 `prototype.md`**；应在 PRD 写清最小交互结论后直接进入 design
...
- **写入**
  - `{FEATURE_DIR}/requirements/prototype.md`（R3 产物，优先按模板生成）
...
- **不生成** `prototype.md`
- 确保 PRD 已补齐最小交互结论（页面与入口、关键控件/字段与校验要点）
- 直接进入 design 阶段
