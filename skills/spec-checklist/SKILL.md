...
- **门禁优先**：只要要读写 `{FEATURE_DIR}/requirements|design|implementation|verification`，必须先满足 **REQUIRED SUB-SKILL：`spec-context`** 并在对话中回显 `FEATURE_DIR=...`（允许 `(reuse)`）。
...
- 若**未给定文件**：执行 **REQUIRED SUB-SKILL：`spec-context`**，并回显 `FEATURE_DIR=...`，然后进入 **步骤 1**。
...
输出一个列表（对话内维护即可）：
- **候选澄清点**：每条包含「文件→章节→原文片段→为什么高价值（阻断/不可逆/风险）」并按优先级排序

### 步骤 2：最小澄清循环（一次只问一个问题）

重复以下闭环：
1. 从“候选澄清点”中选择 **1 条最高优先级**
