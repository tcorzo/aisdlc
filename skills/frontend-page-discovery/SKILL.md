...
目标：**只基于代码证据**，先产出“页面清单（Page Inventory）”，再从页面反推功能点、业务流程与业务规则，帮助产品/研发快速理解项目并沉淀为可复核的文档资产（默认落盘到 `docs/`）。

面向产品阅读的输出原则：
- **先可读**：主干只保留“页面是什么/做什么/入口是什么/关键规则是什么”。技术字段与 `file:lines` 下沉到附录。
- **先模块化**：页面多时，索引按菜单模块分节展示，避免一张超长表格难以浏览。
- **仍可追溯**：任何关键结论要么有证据，要么进入 Evidence Gaps（禁止脑补）。
...
- `docs/frontend/pages/<page_id>.md`
  - 单页（产品优先）：页面概览、核心任务、关键规则；技术映射与证据在附录
- `docs/frontend/flows/index.md`
  - 跨页面业务流程索引（产品视角 + 证据附录）
- `docs/frontend/conflicts.md`
  - 冲突并存清单（产品可读：冲突点/影响/确认问题；证据附录）
...
使用方式：
- 将下述模板**复制**到目标仓库的对应 `docs/` 路径后再填写（模板仅提供结构，不代表事实结论）。
- 若目标仓库没有 `docs/` 目录，可按需调整落盘目录，但建议保持相同的相对结构以便审计与引用。
...
> 模块分组口径（产品可读）：
> - 页面索引的 `module_name` 优先来自菜单分组（一级/二级菜单均可）。
> - 无法从菜单证据映射到页面时，页面进入“未归类”，并在 Evidence Gaps 写清楚“缺少模块归属证据”。
> - 不允许拍脑袋给模块名。
...
- **page_name**：页面名称（**优先来源：页面 UI 标题 / Document.title**；缺失必须进入 Evidence Gaps）
- **module_name**：模块名称（按菜单分组；缺失则归入“未归类”并写缺口）
