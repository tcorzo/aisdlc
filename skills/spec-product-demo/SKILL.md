...
- **不要用在**
  ...
  - `{FEATURE_DIR}/requirements/prototype.md` 缺失 → **停止并回到 R3**
...
### 1) REQUIRED SUB-SKILL：先执行 `spec-context` 并回显 `FEATURE_DIR=...`
...
| “prototype 还没写/找不到，就先做个占位 Demo” | 自创页面与流程，导致不可追溯 | `prototype.md` 缺失 → **回到 R3**；R4 禁止替代 R3 的交互决策 |
