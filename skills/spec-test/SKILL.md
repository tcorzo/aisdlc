...
任一 `spec-test` / `spec-test-*` 完成时，回答末尾必须追加以下两段（不要省略），以便 `using-aisdlc` 自动推进：

- 「本阶段产物已落盘。请**立即调用** `using-aisdlc` 路由下一步（Router 默认自动续跑；若触发硬中断会停下并输出候选下一步）。」
- `ROUTER_SUMMARY`（YAML；以本仓库 Router 协议为准）：

```yaml
ROUTER_SUMMARY:
  stage: Verification
  artifacts:
    - "{FEATURE_DIR}/verification/<generated-file>.md"
  needs_human_review: false
  blocked: false
  block_reason: ""
  notes: "verification 产物已生成/更新；下一步由 using-aisdlc 路由（例如 V2/V3/V4 或回到实现侧）"
```

> 兼容说明：历史上某些文档会使用自由文本/自定义字段形式的“router summary”。在本仓库 Spec Pack 流程中，以 `using-aisdlc` 定义的 YAML `ROUTER_SUMMARY` 为唯一权威输出格式。
