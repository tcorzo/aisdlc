...
ssd/
  ...
  project/                     # 项目级 SSOT（长期资产）
    ...
    architecture/              # 架构资产（稳定）
      ...
      adr/                     # ADR 索引与归档
    components/                # 应用组件地图与模块页（稳定）
      index.md                 # 组件索引（地图层，只导航）
      <module>.md              # 模块页（单页 SSOT；包含 `## API Contract`/`## Data Contract`）
    ops/                       # 运行资产（稳定）
      runbook.md               # 运行手册（总览）
...
- 执行"合并回项目层"的清单（见第 6 节）：
  - ADR 归档到项目层
  - 契约更新到项目层对应组件页（`project/components/<module>.md`）
  - 运行资产更新到项目层 ops/
  - NFR 预算更新到项目层 nfr.md
...
- **对外契约（默认合并到组件页）**：
  ...
  - UX：关键流程/信息架构变更，更新对应组件页的 UX 入口段落（如适用；或以 ADR 记录）
...
> 注：本方案默认不维护单独的 `project/contracts/**` 目录；契约作为长期资产，统一沉到 `project/components/<module>.md` 的契约段落中，并以“权威入口 + 不变量摘要 + 证据链 + 缺口清单”形式治理。
