---
title: Spec 级需求分析阶段 AI 提效方案（输出（可选）PRD +（可选）原型 +（可选）Demo）
status: draft
...
- **PRD**：可评审、可验收、可拆解、可追溯
- **原型（可选）**：当需求涉及新增/变更交互、或交互复杂/不确定时生成；在 `prototype.md` 中落盘任务流、页面结构、节点编号与 AC 映射，可进入 UI 设计与研发
- **可交互 Demo（可选）**：当需要更高保真验证（走查/可用性验证/干系人对齐）时，基于 `prototype.md` 的页面清单与交互说明，为**当前需求**生成独立可运行的交互 Demo 工程（见 R4）
...
> 说明：`spec-context` 的底层实现依赖脚本函数 `Get-SpecContext`（位于 `.aisdlc-cli/scirpts/spec-common.ps1`），其职责是读取当前分支并验证 `{num}-{short-name}` 与 spec pack 目录结构的一致性。
...
|---|---|---|---|
| R0 | `spec-init`（`skills/spec-init/SKILL.md`） | `requirements/raw.md` | → R1：`spec-product-clarify` |
| R1 | `spec-product-clarify`（`skills/spec-product-clarify/SKILL.md`） | `requirements/solution.md` | →（默认）R2：`spec-product-prd` / 或（简单需求）直接进入 design 阶段 |
| R2 | `spec-product-prd`（`skills/spec-product-prd/SKILL.md`） | （可选）`requirements/prd.md` | →（按需）R3：`spec-product-prototype` / 或直接进入 design 阶段 |
| R3 | `spec-product-prototype`（`skills/spec-product-prototype/SKILL.md`） | （可选）`requirements/prototype.md` | → 原型评审/最小可用性验证 →（按需）R4：`spec-product-demo` → 回流更新 R1/R2/R3 |
| R4 | `spec-product-demo`（`skills/spec-product-demo/SKILL.md`） | Demo 工程 | → 运行 Demo 走查/验证 → 回流更新 R1/R2/R3/R4 |

> 说明：Skill 的规范定义以 `skills/` 下对应 `SKILL.md` 为准；本文档负责端到端总览与落盘约定。

#### 3.3. 最短路径（建议先跑通）

`spec-init` → `spec-product-clarify` →（简单需求可直达）design 阶段

或（常规路径）

`spec-init` → `spec-product-clarify` → `spec-product-prd` →（按需）`spec-product-prototype` →（按需）`spec-product-demo` → design 阶段
